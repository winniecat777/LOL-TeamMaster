import{a as m,l as b}from"./main-BXgeMJb9.js";const x=async()=>{try{const n=document.querySelector("#teamListBox");let r=[],g="";const u=(await axios.get(`${m}/teams?_expand=user`)).data;for(const d of u){let l=[],e=[],i=[],a=[],s=d.teamMerberId.filter(o=>o>0);(await axios.get(`${m}/teamsMember/${d.id}`)).data.forEach(o=>{o===null?e.push(0):e.push(o.avatar),o===null?l.push(0):l.push(o.username),o===null?i.push(0):i.push(o.userRank),o===null?a.push(0):a.push(o.likeHero)}),g+=`
      ...
      </div>`}r.innerHTML=c}catch(r){console.error("Error fetching data:",r)}};document.querySelector("#thumbDesc").addEventListener("click",()=>{v("desc")});document.querySelector("#thumbAsc").addEventListener("click",()=>{v("asc")});document.querySelector("#dateLte").addEventListener("change",()=>{const n=document.querySelector("#dateLte").value;k(`${n}`),console.log(n)});document.querySelectorAll("#rankSelect li").forEach(n=>{n.addEventListener("click",r=>{y(r.target.dataset.rank)})});document.querySelector("#teamQuery").addEventListener("change",n=>{f(`${n.target.value}`)});x();
